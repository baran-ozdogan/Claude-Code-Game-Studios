...
> **Correction (empirical, 2026-08-10)**: This ADR's Edit-Mode-testability framing (Consequences → Positive, below) was read by the first implementation pass as implying `Awake()` fires synchronously on `AddComponent()` regardless of Play Mode — it does not. Empirically confirmed (isolated `[Test]` run, then a throwaway diagnostic `MonoBehaviour`): Unity never calls `Awake()` outside Play Mode, not even in Edit Mode, not even after multiple `[UnityTest]` `yield return null` frames, unless the component carries `[ExecuteAlways]` (which `PlayerStateProvider` does not and should not). The movement-lock API (`RequestMovementLock`/`ReleaseMovementLock`/`EffectiveScope`) is unaffected — those tests call instance methods directly and never depend on `Awake()` having run, so they stay in Edit Mode (`Tests/EditMode/fpc_player_state_lock_test.cs`). Only the duplicate-instance guard depends on `Awake()`; its test moved to Play Mode (`Tests/PlayMode/fpc_player_state_duplicate_guard_test.cs`, `[UnityTest]`, same pattern as `UIRootStaleInstanceTest`). Validation Criteria below updated accordingly.
...
### Positive
- `IPlayerState`/the movement lock is genuinely unit-testable in isolation (`AddComponent<PlayerStateProvider>()` on a bare `GameObject` in an Edit Mode test — no live `CharacterController`/`Camera` required, corrected wording per TD-ADR review) — satisfies `coding-standards.md`'s BLOCKING dependency-injection requirement for what is arguably this project's single most-depended-upon piece of state. **Scope note (2026-08-10 correction)**: this applies to the movement-lock API specifically; the `Awake()`-driven duplicate-guard is not Edit-Mode-testable (see top-of-document correction note) and lives in a separate Play Mode test.
...
- A `[UnityTest]` in **Play Mode** (`Tests/PlayMode/fpc_player_state_duplicate_guard_test.cs` — not Edit Mode, see 2026-08-10 correction note above) confirms the duplicate-instance guard: instantiating a second `PlayerStateProvider` while one already exists results in the second being destroyed and `Current` still pointing at the original — verifies the corrected `Debug.LogError`-and-destroy mitigation actually works, not just the previously-insufficient `Debug.Assert`.
