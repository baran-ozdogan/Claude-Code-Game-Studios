...
| `anlati-durum-ipucu-takibi.md` | Orphaned-`shiftId` detection, non-blocking `Debug.LogWarning` | ~~Editor-only via `EditorSceneManager.sceneOpened`/`sceneSaved` + `[InitializeOnLoad]` (user decision, 2026-08-06)~~ → **build-time `IBuildCheckAggregate` yürüyüşü** (user decision, 2026-08-10; tek-sahne tetikleyici proje-geneli iddiayı veremiyordu — addendum bekliyor) |
