...
// Date: 2026-08-02
//
// Extends the original lighting-only MemoryTrigger (prototypes/yankilar-lighting-concept)
// to also fire the stinger audio in sync with the lighting shift — this is the
// entire hypothesis under test, so the two must start on the same frame.
...
public class MemoryTrigger : MonoBehaviour, IInteractable
{
    ...
    [Tooltip("Fires in sync with the lighting shift — this is the compound effect under test.")]
    public StingerVoice stingerVoice;
    ...
    public void OnInteract()
    {
        ...
        stingerVoice?.PlayStinger(transform.position);
