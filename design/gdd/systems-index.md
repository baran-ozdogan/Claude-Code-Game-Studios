...
12. Sahne Kesmeli Anlatı — depends on: Seviye/Sahne Geçişi, Diyalog İçeriği, Gece/Oturum Durumu, Görev/Taşıma Döngüsü, Birinci Şahıs Kontrolcü (**added 2026-08-03** — Full-scope movement lock before HARD CUT). **Anlatı Durum/İpucu Takibi removed 2026-08-03** — saturation signal switched from `OnClueKnown` to Gece/Oturum Durumu's `SettledTriggerIds.Count` (design-review, 2026-08-04 ikinci tur bulgusuyla `FiredTriggerIds.Count`'tan düzeltildi — saturation-timing fix'inin bir parçası, bkz. `gece-oturum-durumu-2026-08-02.md`/`sahne-kesmeli-anlati-2026-08-02.md` Core Rules), this system no longer queries Anlatı Durum at all. **Anı-Tetikleyici Etkileşim corrected 2026-08-04** (verification finding) — this row previously listed it as a direct "same-tier dependency", contradicting the Systems Enumeration table below (row 12, "indirect/event-driven, no direct call"), `sahne-kesmeli-anlati-2026-08-02.md`'s own Dependencies section (which never lists it), and `ani-tetikleyici-etkilesim.md`'s own text ("bu sisteme doğrudan bağımlı değil"). Three of four documents agreed: **no direct dependency** — Sahne Kesmeli reads the saturation signal from Gece/Oturum Durumu's `SettledTriggerIds`/`OnTriggerSettled`, decoupled from Anı-Tetikleyici entirely. This row was the outlier and is now corrected to match.
...
- [x] **`/review-all-gdds` re-run #2 (2026-08-04, same session)** — 5 blocking issues, all of them gaps in round 1's own fixes (2 propagation gaps: stale `FiredTriggerIds`/`OnTriggerFired` references and 2 stale document headers; 3 incomplete fixes: the saturation-timing guard only covered condition (b) not the symmetric (a) race, the two-endings differentiation only touched audio not movement-lock scope, and the new mandatory Automatic zone would have incorrectly retriggered the memory-trigger stinger). All fixed same session.
- [x] **`/review-all-gdds` re-run #3 (2026-08-04, same session)** — 3 blocking issues, narrower still: a third file (`anlati-durum-ipucu-takibi.md`, status Approved) carrying the same stale `FiredTriggerIds` reference that keeps recurring; `sahne-kesmeli-anlati-2026-08-02.md`'s own Dependencies section and this file's Dependency Map both still asserted the pre-differentiation unconditional `Full` lock; and one genuinely new finding — `diyalog-anlati-icerigi-2026-08-02.md`'s callback-selection timing was ambiguous enough (`Awake`/`Start` vs. post-`Swapping`) that an implementer could silently reintroduce the saturation-timing bug through a different mechanism. Also closed an ending-tie-break ambiguity Phase 3 surfaced (both endings can now become satisfiable from the same event — added explicit precedence: saturation wins). All fixed same session.
- [x] **User decision: stop the re-verification loop here (2026-08-04)** — blocking-issue count dropped 8 → 5 → 3 across three rounds, and round 3's findings were narrower/more subtle (a stale reference, a timing-order question, a tie-break gap) rather than new fundamental design problems — a credible convergence signal. User explicitly chose not to run a 4th pass. **Status fields deliberately NOT flipped to Approved** — per this project's own discipline, that requires an actual clean verification pass, which was not run. Any further subtle gaps are expected to surface during architecture/vertical-slice work rather than another design-only pass; this is an accepted, explicit risk tradeoff, not an oversight.
