...
- [ ] **Constructor-time abonelik**: `AnlatiDurumState` constructor'ı `IsikVolumeDurumSistemi.Instance.OnShiftStateChanged`'e abone olur — first-access zamanı, sahne-lokal bir `Awake`/`OnEnable` DEĞİL (GDD Core Rules "Abonelik zamanlaması", TR-anlati-006); abonelik süreç başına bir kez bağlanır, `ResetOnLoad()` onu yeniden-wire ETMEZ
...
- **AC-2 (otomatik)**: Constructor aboneliği
  - Given: `AnlatiDurumState` kurulur
