...
## Session Extract — /dev-story + kapanış, anlati Story 002, 2026-08-10
- Verdict: COMPLETE WITH NOTES → **Status: Complete** (epic 2/5)
- Files: `ClueDefinition.cs` + `ClueRegistry.cs` (YENİ, iki `[CreateAssetMenu]` SO), `AnlatiDurumState.cs` (+ters indeks/`BuildReverseIndex`/`BindEnsureRegistryLoaded`/`IsRegistryLoaded`/`ProcessShiftStateChanged`/`ProcessHeldShift`), `Tests/EditMode/anlati_clue_definition_test.cs` (YENİ, 25 test)
- **Story sınırı dikişi TUTTU**: ADR-0007 `EnsureRegistryLoaded()` (Addressables) ile Held mantığını tek metotta birleştiriyordu; enjekte edilebilir `Action` kancasına ayrıldı (Held filtresi → kanca → `ProcessHeldShift`). LP, ADR'ın sıralama semantiğinin birebir korunduğunu ve Story 003'e saf ekleme bıraktığını doğruladı.
- **LP düzeltmeleri**: `[CreateAssetMenu]` menü kökü `"Yankılar/"` (dotless-ı) yazılmıştı, mevcut `ShiftConfig` `"Yankilar/"` (ASCII) kullanıyor → Unity İKİ ayrı Create menüsü çizerdi; hizalandı. `ClueRegistry.EditorSetDefinitions` SİLİNDİ (Assets/Scripts'teki tek `#if UNITY_EDITOR` dikişi, sıfır çağıran, SO'nun serialized alanına canlı alias veriyordu) — testler `SerializedObject` desenini kullanıyor.
- **QL test eklemeleri**: boş `RequiredShiftIds`'in çalışma zamanı davranışı pinlendi (GDD'nin en gürültülü tehlikesi — `SeenShiftIds ⊇ ∅` her zaman doğru; bugün yapısal olarak imkânsız ama hiçbir test tutmuyordu); abone patlamasının çok-aday döngüsündeki sonucu; `ClueRegistry`'nin kendi sözleşmesi (sıfır kapsamı vardı); `BuildReverseIndex` ikinci çağrıda birleştirmiyor değiştiriyor; 3+ gereksinimli ipucu; `RequiredShiftIds` içindeki boş girdi; canlı-görünüm sonucu #2 (`InvalidOperationException`).
- **AC-1'in kendi testi ayırt edici değildi**: `Held("A")` ile başlıyordu, "kovada yoktu" ile "eksikti"yi ayıramıyordu ve başka bir testle birebir aynıydı → önce `Held("B")` sonra `Held("A")` kurgusuna çevrildi (iki ipucunun da tamamlanması yalnız A'nın kovası her ikisini de taşıyorsa mümkün).
- **Story 003'e devredilen iki karar**: (1) `IsRegistryLoaded` "indeks var" demek, "yükleme başarılı" değil — `WaitForCompletion()` patlarsa bloklayan çağrı her Held'de yeniden denenir, latch'leme kararı orada verilmeli; (2) abone istisnası canlı `OnShiftStateChanged`'e bağlanınca Işık/Volume multicast'ine düşer, diğer aboneleri düşürebilir.
- Süit: **EditMode 182/182** (ilk koşu 175/175 temiz). PlayMode değişmedi (78/78).
- Next: Story 003 / 004 / 005 (üçü de 002'ye bağlı, birbirine değil — paralel). Commit yok (talimat bekliyor).

## Session Extract — /dev-story + kapanış, anlati Story 001, 2026-08-10
- Verdict: COMPLETE WITH NOTES → **Status: Complete** (epic 1/5)
- Files: `Foundation/AnlatiDurumIpucuTakibi/` (YENİ klasör — `IAnlatiDurumState.cs`, `AnlatiDurumState.cs`, `AnlatiDurumIpucuTakibi.cs`), `FoundationBootstrap.cs` (TODO → gerçek kayıt, Işık/Volume+Gece/Oturum'dan SONRA), `foundation_bootstrap_order_test.cs` (+1 satır), `Tests/EditMode/anlati_durum_facade_test.cs` (YENİ, 18 test)
- **EN ÖNEMLİ BULGU (LP) — abonelik yeri iki farklı yerde belgeleniyordu**: ADR-0007 aboneliği `AnlatiDurumState` CONSTRUCTOR'ına koyuyor; bu kod tabanı için YANLIŞ. Testler ADR-0001 gereği taze State kurar (18 testin hepsi `[SetUp]`'ta) → State-ctor aboneliği her testte kalıcı `OnShiftStateChanged`'e bir handler daha sızdırır, "saf state" testlerini gerçek facade'a bağımlı kılardı (manifest ihlali). Proje emsali `GeceOturumDurumu` de FACADE static ctor kullanıyor. Kod her ikisiyle uyumlu (constructor henüz yok — sapma additive); düzeltme BELGEDE: Story 003 AC'si + test şekli facade static ctor'a çevrildi. **ADR-0007 addendum'u gerekiyor** (Story 005'in mekanizma sapmasıyla aynı pakette).
- Diğer LP düzeltmeleri: `internal HashSet SeenShiftIds` ham koleksiyon sızdırıyordu → `MarkShiftSeen`/`SeenShiftIds` (salt-okunur)/`HasSeenShift`; `MarkClueKnown(null/boş)` sessiz no-op (yazılmamış ClueDefinition alanı koleksiyona null sokup tüketicide NRE üretirdi); canlı-koleksiyon dönüşünün iki sonucu XML doc'a yazıldı; `OnClueKnown` abonelerinin reset'te bilerek temizlenmediği belgelendi.
- QL test eklemeleri: abonenin reset'i sağ geçmesi + reset sonrası yeniden-işaretlemede event'in TEKRAR fırlaması (in-place rejimin varoluş sebebi; gece-oturum emsalinde vardı, burada eksikti); abone patlarsa yazma geri alınmaz; canlı-görünüm sabitlendi; null guard testi; facade reset testine negatif kontrol + `finally` (önceden totolojik geçebilirdi).
- **AC-7 reflection deliklerinin kapatılması (QL)**: `Static` üyeler, nested tipler ve **taban arayüzler** — sonuncusu en gerçekçi kaçaktı (`IAnlatiDurumState : IClueOrdering` yazılsa kapalı-yüzey testi hâlâ "tam 4 üye" görüp geçerdi). `GetInterfaces()` boş assert'i eklendi.
- Süit: **EditMode 158/158** (ilk koşu 153/153 temiz, gate fix'leriyle 158). PlayMode değişmedi (78/78 — bu story'de MonoBehaviour yok).
- Next: Story 002 (`story-002-cluedefinition-ters-indeks-held-mantigi.md`). Commit yok (talimat bekliyor).
...
- **BLOCKING BULGU: sahne adları YANLIŞTI** — `{"Depo","ServisKoridoru","BaloSalonu"}` yazılmıştı, mimarinin kilitli adları İngilizce: `Depot`/`Ballroom` (control-manifest Scenes/Prefabs satırı, ADR-0015 `_initialLevelSceneName="Depot"`, ADR-0011, ve mevcut `build_validation_harness_test.cs` zaten `Depot.unity` kullanıyor). Check gerçek sahneleri HİÇ tanımaz, sonsuza dek sessizce geçerdi. Ayrıca **Servis Koridoru bir sahne değil** — ADR-0011 MVP'si tam olarak 2 kat; koridor bir kat sahnesinin içinde (per-scene granülerlik onu ifade edemez, bilinçli sınır olarak belgelendi).
- **TRIPWIRE eklendi** (iki gate de bağımsız istedi): `fpc_decoy_scene_drift_test.cs` — Build Settings'e giren her sahne ya muaf ya tanınan MVP alanı olmalı; tanınmayan sahne eklendiği an kırmızı. "Sıfır sahne sessizliği" (doğru) ile "başka adla gelen sahne sessizliği" (fail-open) farkını kapatır. Ad listesi ayrıca literal pinlendi (eski test listeyi kendine karşı geziyordu — totolojik).
- **AC-1 bilinçli daraltıldı**: boş `PromptText` artık build'i kırıyor. Etkileşim crosshair'i prompt çizer — promptsuz decoy "yavaşlama + yazı yok = decoy" kuralını öğretir, kamuflaj oyuncunun GÖRDÜĞÜ katmanda sızar. GDD "minimal/**tatsız** tepki" diyor, tepkisiz değil.
- Diğer: case-insensitive sahne eşleşmesi (Windows'ta `depot.unity` sessizce atlanırdı); dosya adı=sınıf adı; reflection kamuflaj taraması Static+İMZA kapsıyor; sahne-başına atıf testi (`FakeWalker.onOpen`); gerçek registry dizisi uçtan uca; **PlayMode'da gerçek `DecoyInteractable`'ın registry kayıt/çıkış döngüsü** (story'nin tüm runtime değeri buydu, hiç test edilmemişti).
- Süit: **EditMode 140/140, PlayMode 78/78**.
- **EPİK BİLANÇOSU (6/6)**: 001 IPlayerState+kilit → 002 saf hareket matematiği → 003 FPC sürücüsü+Input+prefab → 004 kalıcı sahne+RepositionTo → 005 taper/carry/faz wiring → 006 decoy build-check. Gate'ler bu epic'te **6 gerçek üretim bug'ı** yakaladı: Awake-edit-mode varsayımı, gamepad bakış ölçeği, paylaşılan InputActionAsset'in hayatta kalanı sakatlaması, Velocity'nin yere-yapıştırma sabitini yayınlaması, `RepositionTo`'nun yanlış imzayla kilitlenmesi, taper `d`'sinin 3B ölçülmesi. Ayrıca 2 yanlış-yeşil test (Physics.autoSyncTransforms, decoy sahne adları).
- Next: sıradaki Foundation epic'i (`anlati-durum-ipucu-takibi` — index'te Ready, ADR-0007) ya da sprint close-out (/smoke-check → /team-qa → /gate-check). Story 006 commit'lenmedi (talimat bekliyor).
...
- **EN ÖNEMLİ BULGU — story'nin kilitleyeceği API imzası YANLIŞTI.** Tek-anchor `RepositionTo(Transform)` mutlak bir snap; SOFT transition ise GÖRELİ (kabin-yerel) süreklilik ister. Üç kaynakla doğrulandı: GDD koordinat-çerçevesi kuralı ("kopyalanan dünya-uzayı değil kabin-yerel pozisyon/rotasyondur"), ADR-0008'in `CopySoftTransitionAnchorTransform(fromScene, toScene)` iki-sahne imzası, ADR-0015 birebir ("relative continuity FROM a source scene"). Asansörde kilit MoveOnly → oyuncunun kabin-içi konumu/bakışı keyfî; snap onu siler ve GDD'nin yasakladığı sıçramayı üretir. **Çözüm: iki aşırı yükleme** (mutlak = ADR-0015 boot spawn `InitialSpawnAnchor`; göreli = SOFT). Yanlış primitifi kilitlemek story'nin amacının tersi olurdu.
- Diğer düzeltmeler: gövde rotasyonu YAW'a düzleştiriliyor (eğik anchor kalıcı+kurtarılamaz gövde eğimi üretirdi — `Space.Self` yaw'ı asla düzeltmez); setup script'inin idempotency kontrolü yıkımdan ÖNCEye alındı. Yeni testler: `_lastMoveDirection` dönüşü (silinse tüm testler yeşildi), ışınlanma-sonrası Velocity sıçraması yok, kilit altında repozisyon, eğik-anchor düzleştirme, göreli-form kabin-yerel koruma, **Story 003 paylaşılan-InputActionAsset bug'ının regresyon testi**.
- **Test altyapısı bulguları (kalıcı)**: (a) `Physics.autoSyncTransforms` bu projede FALSE — kod içinden kurulan test collider'ları PhysX'e senkronlanmazsa ORİJİNDE 1x1x1 kalır; Story 003'ün duvar testleri bu yüzden yanlış bir dünyada koşuyordu ve "çarpışma oldu" sayacı yanlışlıkla artıyordu (`Physics.SyncTransforms()` eklendi — GERÇEK bir yanlış-yeşil idi). (b) `PrefabUtility.IsPartOfPrefabInstance` PlayMode'da FALSE döner (prefab bağlantısı Editor-only metadata) — o iddia EditMode'a taşındı.
- Süit: **EditMode 120/120, PlayMode 61/61**. CI: c07f69f **YEŞİL** (run 31382913227).
- Next: Story 005 (taper/carry/faz wiring) → 006 (decoy build check). Story 004 değişiklikleri commit'lenmedi (talimat bekliyor).
